...
df = pd.read_csv(r'C:\salma\TETRIS\CAPSTONE\dataset\combine_table_withzero.csv')
#print(df.head())
# Sidebar menu
...
if selected_menu == "Home":
    ...
    st.markdown(
    "The dataset used in this analysis is collected from the JobStreet API. The data includes job listings related to 'Data Scientist' with specific parameters."
    )
    st.subheader("Data Visualization")
    st.markdown(
        "Explore job opportunities in a visually rich manner with our Data Visualization feature. Leveraging data from the JobStreet API, specifically targeting roles related to Data Scientists, this website provides dynamic visual representations. Delve into interactive charts and graphs to uncover trends and patterns within the dataset. Key columns such as job title, company, category, salary, ratings by star, requirements, date uploaded, and location are visually presented, allowing you to make data-driven decisions in your job search."
    )
    ...
    st.markdown(
        "Gain insights into company performance through our 'Rank Companies by Star' functionality. Evaluate and compare companies based on their star ratings, derived from specific criteria and performance metrics. This feature streamlines your company research process, offering a quick snapshot of top-rated employers. Make informed decisions about potential employers and find the best-fit company for your career aspirations."
    )

    st.subheader("Category Prediction from Job Requirements using Machine Learning")

    st.markdown(
        "Experience the future of job search with 'Category Prediction from Job Requirements' powered by machine learning of logistic regression and random forest. Input job requirements, and let our machine learning models predict the most relevant job category. This feature simplifies the job categorization process, providing quick and accurate classifications based on stated requirements. Enhance your job search efficiency with the innovative capabilities of machine learning."
    )
